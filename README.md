...
| **改行削除1** / **改行削除2** / **改行戻す** | 3状態トグル。改行削除1: 段落保持で改行削除。改行削除2: 段落区切りも削除。改行戻す: 元テキストに復元 |
| **VFM** | AIを使ってテキストを[Vivliostyle Flavored Markdown (VFM)](https://github.com/vivliostyle/vfm)に変換。見出し・リスト・引用・脚注を自動検出。改行削除1または改行削除2の適用後に使用可能。**注意:** 変換精度はAIモデルに依存します。期待通りの結果が得られない場合は、別のAIモデルへの変更や、プロンプトのカスタマイズをお試しください |
| **コピー** | 現在のテキストをクリップボードにコピー |
| **保存** | 現在表示中のテキストをTXTファイルとしてダウンロード（結合モードでは結合テキスト全体を保存） |
| フォントサイズ | スライダーでテキストのフォントサイズを調整（10〜24px） |
| ファイル名を記載 | 保存時にファイル名を先頭に付加 |
...
| 結合表示 | 複数選択時、右パネルに結合テキストを表示（「N件を結合表示中」バッジ）。結合状態で編集・AI校正・コピー・保存が可能 |
