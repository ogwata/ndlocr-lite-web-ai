...
- **改行削除トグル** — 段落保持の改行削除 → 全改行削除 → 元テキストの3状態をワンクリックで切替
...
6. 「コピー」「保存」ボタンで現在表示中のテキストを出力

### 改行削除の使い方

OCR結果はブロックごとに改行されています。「改行削除」ボタンで段落内の改行を除去し、読みやすいテキストに変換できます。

| ボタン表示 | 動作 |
|-----------|------|
| **改行削除1** | 段落区切りを保持して、段落内の改行のみ削除。タイトル・見出し・短い行の改行は保持されます |
| **改行削除2** | 段落区切りも削除して、本文全体を連続テキストに結合します（リーダー線は保持） |
| **改行戻す** | 元のOCRテキスト（改行削除前）に戻します |

ボタンをクリックするたびに「改行削除1 → 改行削除2 → 改行戻す」と切り替わります。VFM変換を使う場合は、改行削除1または改行削除2を適用してから実行してください。
...
- feat: 改行削除を3状態トグルに変更（改行削除1: 段落保持 → 改行削除2: 段落なし → 改行戻す）
- feat: TXTと一括TXTを「保存」ボタンに統合

### v0.13.0（2026-03-26）

- feat: 編集テキストを結果IDごとにキャッシュし、単一↔結合モード切替時に編集内容を復元
- fix: 改行削除・AI校正適用・VFM変換等の操作でも破棄確認ダイアログが正しく表示されるよう修正
- fix: 結合モードのAI校正をレートリミット回避のため3件ずつ順次処理に変更

### v0.12.6（2026-03-26）

- feat: AI校正中に経過時間を表示（「AI校正中... 8/9（45秒）」形式）

### v0.12.5（2026-03-26）

- fix: 結合モードの改行削除でもタイトル・見出し・リスト項目の改行を保持するよう修正

### v0.12.4（2026-03-26）

- fix: 改行削除でタイトル・見出し・リスト項目の改行が誤って削除される問題を修正（行テキスト長の中央値から短い行を判定し改行を保持）

### v0.12.3（2026-03-26）

- fix: 単一画像↔結合モードの切り替え時、編集済みテキストが警告なく破棄される問題を修正（全切り替えパターンで確認ダイアログを表示）

### v0.12.2（2026-03-26）

- fix: 長いテキストのAI校正でエラーになる問題を改善（max_tokens 4096→8192、タイムアウト 60秒→120秒）
- fix: AI校正エラー時に詳細メッセージを画面に直接表示（従来はホバーのみ）
